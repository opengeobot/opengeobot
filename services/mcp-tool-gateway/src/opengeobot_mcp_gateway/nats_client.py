# Function: NATS connection management for the MCP tool gateway
# Time: 2026-07-06
# Author: AxeXie
...
class NatsBridge:
    ...
    ensured so that mission-critical subjects (``opengeobot.mcp.>``) are
    ...
    async def connect(self) -> None:
        ...
        async def _disconnected_cb() -> None:
            ...
            logger.warning("NATS connection lost; MCP gateway in standby")
        ...
        async def _reconnected_cb() -> None:
            self._connected.set()
            logger.info("NATS reconnected; MCP gateway resuming")
    ...
    async def ensure_stream(self) -> None:
        """Create or verify the JetStream durable stream for MCP subjects.

        The stream covers ``opengeobot.mcp.>`` so that mission-critical
