# Function: ROS2 skill unit tests
# Time: 2026-07-15
# Author: AxeXie
"""Unit tests for the ROS2 skill implementations (F-ADAPTER-003).

These tests run without rclpy installed. The skills fall back to simulation
mode, returning success without publishing to real ROS2 topics.
"""
...
from opengeobot_ros2.skills import (
    CaptureImageSkill,
    EmergencyStopSkill,
    MoveForwardSkill,
    Skill,
...
from opengeobot_ros2.skills.base import SkillContext, SkillResult
from opengeobot_ros2.skills.move_forward import MAX_DISTANCE_METERS, MAX_SPEED_MPS
...
class TestSkillProtocol:
    def test_all_skills_satisfy_protocol(self) -> None:
        for skill in [
            StandUpSkill(), StopSkill(), MoveForwardSkill(),
            CaptureImageSkill(), EmergencyStopSkill(),
        ]:
    ...
    def test_default_skills_with_node_none(self) -> None:
        """default_skills(node=None) must work without rclpy."""
        skills = default_skills(node=None)
        assert len(skills) == 5
...
class TestCaptureImageSkill:
    async def test_success(self) -> None:
        ...
        assert result.success is True
        assert result.output["image_path"].startswith("/ros2/captures/")
