# Function: NATS connection management for the Safety Gateway
# Time: 2026-07-06
# Author: AxeXie
...
asyncio events so the Safety Gateway can operate independently of the cloud.
The Safety Gateway must remain functional when the NATS connection is lost —
emergency stop latching and safety state are preserved locally regardless of
connection state.
"""
...
class NatsBridge:
    ...
    async def connect(self) -> None:
        ...
        async def _disconnected_cb() -> None:
            ...
            logger.warning("NATS connection lost; Safety Gateway continues in standalone mode")
        ...
        async def _reconnected_cb() -> None:
            self._connected.set()
            logger.info("NATS reconnected; Safety Gateway resuming online mode")
        ...
        async def _closed_cb() -> None:
            self._connected.clear()
            self._closed.set()
            logger.error("NATS connection closed permanently; Safety Gateway remains operational")
        ...
        self._nc = await nats.connect(
            ...
            name=f"safety-gateway-{self._config.gateway_id}",
