# Function: NATS connection management for the edge gateway
# Time: 2026-07-05
# Author: AxeXie
"""NATS connection management with auto-reconnect and lifecycle hooks.

nats-py provides native reconnect support; this wrapper wires its callbacks to
asyncio events so the gateway can react to disconnect/reconnect for offline
caching and reconciliation. The reconnect schedule uses a bounded exponential
backoff up to ``max_reconnect_wait`` seconds.
"""
...
class NatsBridge:
    ...
    async def connect(self) -> None:
        ...
        async def _disconnected_cb() -> None:
            ...
            logger.warning("NATS connection lost; entering offline mode")
        ...
        # nats-py implements exponential backoff internally via max_reconnect_attempts
        # and reconnect_time_wait; -1 attempts means retry indefinitely.
        max_reconnect = self._config.nats_max_reconnect
        self._nc = await nats.connect(
            servers=self._config.nats_url,
            name=f"edge-gateway-{self._config.robot_id}",
    ...
    async def request(self, subject: str, data: bytes, timeout: float) -> Msg:
        """Send a request and await the reply. Used for edge → executor calls."""
        if self._nc is None:
            raise NatsConnectionError("NATS client not connected")
